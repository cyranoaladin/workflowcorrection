...
def upgrade() -> None:
    ...
    op.create_table(
        "copy_pages",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("copy_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("student_copies.id", ondelete="CASCADE"), nullable=False),
    ...
    op.create_index("uq_copy_pages_copy_id_page_number", "copy_pages", ["copy_id", "page_number"], unique=True)

    op.create_table(
        "transcriptions",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True, nullable=False),
        sa.Column("copy_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("student_copies.id", ondelete="CASCADE"), nullable=False),
        sa.Column("page_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("copy_pages.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("correction_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("confidence", sa.Numeric(), nullable=True),
        sa.Column("needs_human_review", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("validated_by_human", sa.Boolean(), nullable=False, server_default=sa.text("false")),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
    )
    op.create_index("ix_corrections_copy_id", "corrections", ["copy_id"])
    op.create_index("uq_corrections_copy_id_question_id", "corrections", ["copy_id", "question_id"], unique=True)
